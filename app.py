    ...
    if search_model:
        ...
        for i in result_as_dict:
            ...
            with st.chat_message("user"):
                result = template_text.format(model_name=model_name,maintainer=maintainer)
...
def llm_leaderboard_page():
    ...
    quick_filters = st.multiselect("Filters ",['Model Name', 'Maintainer', 'License', 'Context Length','Mt Bench', 'Humaneval','Input Priceusd/1M Tokens'],
    ...
    # Top N Models
     ...
     # Select the top 10 models by metric
    top_models = df_leaderboard.nlargest(columns=metric,n=10)[["Model Name","Maintainer",metric]]
...
llm_leaderboard = st.Page(llm_leaderboard_page,title="Leaderboard",icon=":material/favorite:")


# Navigations
pg = st.navigation({"Home":[llm_list,ask_llm,llm_stats,llm_leaderboard],
                    "About":[about]})
